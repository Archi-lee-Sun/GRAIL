...
### Backend
The database schema, system architecture, and overall backend structure were designed collaboratively between the developer and Claude (Anthropic). The vast majority of the backend code—encompassing controllers, database queries, routing, services, and core business algorithms—was handwritten by the developer. In the final phases of the project, during rigorous testing, the developer identified a series of bugs and leveraged OpenAI Codex to target and resolve these specific issues. As a result, the final backend codebase features minor, targeted modifications generated by AI under the developer's direct supervision to ensure system stability.
